...
understanding the civilization's resilience and eventual decline. Analysis of the complete 
unfiltered per-type frequency data independently confirms power law 
structure ($p = 0.71$), validating the guild scale-free hypothesis across both constrained 
and complete methodologies. Exact per-type frequency data would further refine these estimates.
\end{abstract}
...
For the guild size distribution, we employed two complementary approaches. First, we analyzed 
the published binned summary data (Table 7.6) by assigning each type the mean frequency of its bin 
(total seals / number of types); this introduces approximation but captures the gross distribution. 
...
we analyzed the complete, unfiltered Appendix 7.1 dataset from \citet{jamison2017} (500 seals, 134 offering stand types). 
This dataset contains the raw frequency counts for each type, eliminating the need for binning 
or reconstruction:

\begin{itemize}
    \item Power law exponent: $\alpha = 2.41$
    \item Lower bound: $x_{\min} = 3.0$
    \item KS distance: $D = 0.0577$
    \item Goodness-of-fit $p = 0.71$ (bootstrap, 2500 iterations) $\checkmark$ PASSES
\end{itemize}

The exponent $\alpha \approx 2.41$ converges closely with the Table 7.6 estimate 
($\alpha \approx 2.36$ from constrained reconstruction), validating the core power law 
hypothesis across both methodologies. The bootstrap goodness-of-fit test passes the 
\citet{clauset2009power} threshold ($p > 0.1$), confirming power law as a plausible 
generative model for the actual per-type distribution. Applying the same bootstrap test to the 
exponential distribution yields $p_{\text{GoF}} < 0.001$ (0/2500), formally ruling out 
exponential as a plausible model for the complete dataset as well.

**Important caveat**: The optimal lower bound $x_{\min} = 3.0$ yields only $n_{\text{tail}} = 47$ 
types in the power law regime, which falls below the $n_{\text{tail}} \geq 100$ threshold 
recommended by \citet{clauset2009power} for reliable parameter estimation. Consequently, 
while the complete data confirms that power law structure exists in the distribution 
($p_{\text{GoF}} = 0.71$), the parameter estimate $\alpha = 2.41$ is preliminary. 
Table 7.6 analysis, with $n_{\text{tail}} = 114$ types, provides more robust exponent estimates. 
The two analyses together demonstrate that power law structure is robust to methodological 
approach, while parameter estimates are best based on the larger tail sample in Table 7.6.

Table~\ref{tab:appendix_compare} summarizes likelihood ratio tests of the complete dataset 
against four alternative distributions. Power law is not ruled out by any alternative 
(minimum $p = 0.108$ for truncated power law), and is most strongly favored over exponential 
($R = 1.25$, $p = 0.66$), though this comparison is less decisive than for the binned data 
due to the smaller tail sample.

\begin{table}[H]
\centering
\caption{Distribution tests for guild size data using complete Appendix 7.1 dataset. 
First row shows exponential goodness-of-fit bootstrap test (rules out exponential). 
Remaining rows show log-likelihood ratio tests: positive $R$ favors power law; negative $R$ favors alternative. 
Note: $n_{\text{tail}} = 47 < 100$ (below recommended threshold).}
\label{tab:appendix_compare}
\begin{tabular}{@{}lrrl@{}}
\toprule
Test & $R$ & $p$ & Interpretation \\
\midrule
Exponential (GoF bootstrap) & — & $<0.001$ & \textbf{Exponential rejected} \\
\midrule
Lognormal (vs PL) & $-0.90$ & 0.340 & No significant difference \\
Exponential (vs PL) & 1.25 & 0.657 & Power law favored, not significant \\
Stretched exponential (vs PL) & $-1.07$ & 0.319 & No significant difference \\
Truncated power law (vs PL) & $-1.29$ & 0.108 & No significant difference \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{appendix71.png}
    \caption{Log-log rank-frequency plot of all 134 offering stand types from the complete dataset \citep{jamison2017}. 
    The linear trend in log-log space confirms power law structure despite the smaller tail sample ($n_{\text{tail}} = 47$).}
    \label{fig:appendix71}
\end{figure}
...
The Appendix 7.1 row provides independent confirmation of power law structure; 
parameter estimate is preliminary due to $n_{\text{tail}} = 47 < 100$.}
\label{tab:comparison}
\begin{tabular}{@{}lcccc@{}}
\toprule
Metric & Guilds (bin-mean) & Guilds (reconstructed) & Guilds (complete) & Sites \\
\midrule
$\alpha$ & 2.18 & 2.49 $\pm$ 0.14 & 2.41 & 1.55 \\
$x_{\min}$ & 1.0 & 1.0 & 3.0 & 2.0 \\
KS $D$ & 0.28 & 0.059 $\pm$ 0.011 & 0.0577 & 0.094 \\
GoF $p$ (bootstrap) & $<0.01$ & 0.91 & 0.71 & 0.76 \\
$n_{\text{tail}}$ & --- & 114 & 47$^*$ & 14 \\
Data type & Binned & Constrained random & Complete unfiltered & Exact \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^*$Below $n \geq 100$ threshold; parameter estimate is preliminary.}
\end{tabular}
\end{table}
...
The guild size analysis proceeded in two complementary methodologies: a constrained reconstruction 
of binned summary data from Table 7.6 (Section~4.1.1), and analysis of the exact per-type frequency 
data from Appendix 7.1 (Section~4.1.2). The constrained reconstruction (Table 7.6, $n_{\text{tail}} = 114$) 
yields more robust parameter estimates, comfortably exceeding the $n \geq 100$ threshold recommended by 
\citet{clauset2009power}. The complete per-type dataset (Appendix 7.1, $n_{\text{tail}} = 47$) independently 
confirms power law structure ($p_{\text{GoF}} = 0.71$), validating our distribution-type conclusion across 
both methodologies, but the smaller tail sample limits the precision of the parameter estimate $\alpha = 2.41$. 

The complete Appendix 7.1 dataset from \citet{jamison2017} contains 134 offering stand types, including Jamison's subclassified 
variants (e.g., Types 40, 40b, 61b), whereas the binned Table 7.6 summary reports 114 aggregate types. 
This finer-grained type system is methodologically advantageous: demonstrating that power law structure 
persists across a more granular type system confirms that the distribution is robust to the level of 
typological detail. Jamison's subclassifications represent meaningful archaeological distinctions; 
preserving these distinctions honors his expertise and captures the full range of recorded variation.
...
investigation and discovery. We have completed analysis of exact per-type frequency data from 
Appendix 7.1 (Section~4.1.2), confirming that power law structure persists in the full unfiltered dataset.
Complementary quantitative validation would include:
\begin{itemize}
    \item Consolidating the 134-type Appendix 7.1 dataset to match the 114-type classification scheme 
    of Table 7.6, verifying that power law structure persists when applying identical typological 
    boundaries across both methodologies. This would provide independent confirmation of the distribution 
    type under unified type definitions and enable direct parameter comparison.
...
\href{https://github.com/dmahesh2869/harappan-seal-analysis}{dmahesh2869/harappan-seal-analysis}. 
The repository includes:

\begin{itemize}
    \item Python analysis scripts (analyze\_guilds.py, analyze\_appendix71.py, analyze\_sites.py, 
    synthetic\_guild.py) implementing the power law fitting and goodness-of-fit tests
    \item Data files (guilds.txt, guilds\_valid.txt, sites\_table.txt)
    \item Generated figures (guilds.png, appendix71.png, sites.png)
    \item VALIDATE.py for reproducibility sanity checks
    \item RUN\_ALL.py for end-to-end pipeline validation
    \item Comprehensive reproducibility guide (REPRODUCIBILITY.md)
    \item MIT License for code and CC-BY-4.0 for documentation
\end{itemize}

All analysis is deterministic and fully reproducible using Python 3.8+ with powerlaw v2.0.0.
...
cascading failure. Parameter estimates are most reliable from Table 7.6 ($n_{\text{tail}} = 114$), while 
the complete dataset independently validates the distribution type. Co-occurrence analysis would 
further strengthen these conclusions.
