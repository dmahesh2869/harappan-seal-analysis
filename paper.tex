...
for Harappan seals in which the unicorn motif serves as a civilizational emblem, the offering 
...
Deriving from the above, we propose that Harappan seals function as a structured information 
system primarily analogous to a modern shipping label, though they likely also served broader 
administrative purposes such as identity badges, securing storage and authenticating transactions:

\begin{enumerate}
    \item \textbf{Unicorn motif}: Civilizational emblem (analogous to a country-of-origin mark 
    or trade authority). The recipient of a sealed package sees a unicorn first, identifying it 
    as Harappan. Stylistic variation in anatomical details (ear, eye, horn, etc.) reflects 
    artisanal variation, not encoded information.
...
\citep{clauset2009power}. The lower bound $x_{\min} = 2$ likely reflects sampling limitations 
at minimally excavated sites rather than a genuine departure from the power law at low counts.
...
structure characteristic of scale-free networks. The evidence rests on three pillars:
...
\begin{enumerate}
    ...
    \item \textbf{No central planning required}: Scale-free networks arise through growth 
    and preferential attachment \citep{barabasi1999emergence}---a self-organizing process in 
    which the network expands as new participants join, and successful guilds attract 
    disproportionately more connections over time. The Harappan Civilization expanded over 
    centuries across more than a million square kilometers, plausibly adding new guilds 
    throughout. This is consistent with the IVC's apparent lack of centralized political authority 
...
\citep{albert2000error}. This framework offers insight into both the longevity and the decline 
of the IVC:
...
production and circulation. Since 43\% of guild types appear only once in the record, these 
marginal producers likely depended on the commercial infrastructure maintained by the hubs. 
Once that infrastructure failed, the economic rationale for seal use---authentication of 
goods in a multi-party trade network---would have diminished. Sparse persistence of seals 
...
If the proposed information architecture is correct---unicorn as emblem, offering stand as 
guild identifier, and script as the remaining variable element---then the script's function 
...
transactional function. Further progress may come from correlating script sequences with 
offering stand variants and site of origin to test whether specific sign combinations 
associate with specific guilds or trade routes. More speculatively, if the information 
architecture proposed here is accepted, it may provide a structured framework for analyzing 
the broader Indus script corpus: signs or sign sequences that co-vary with offering stand 
type might encode guild-specific content, while those that vary independently could represent 
quantities, designations, or other transactional and administrative fields. Such an approach would not 
constitute decipherment in the traditional sense, but it could begin to identify the 
functional roles of sign positions within the inscription---distinguishing, for instance, 
``what is being recorded'' from ``how much'' --- even without phonetic readings. We stress that 
this remains a hypothesis to be tested against the data, not a claim.
...
commodity control rather than proper nouns, and that seal iconographies represent guild 
emblems---both findings that align directly with the information architecture proposed here.
...
Although the guild interpretation receives independent support from the power law's 
distribution type (which argues against purely artistic variation), its concordance with 
modern firm size distributions \citep{axtell2001zipf}, and the compatible findings of 
\citet{mukhopadhyay2023}, direct archaeological evidence linking specific offering stand 
variants to specific production or trade groups remains to be established.

\subsection{Future Work}

Access to exact per-type frequency data would enable:
\begin{itemize}
    \item A direct power law test for guild sizes, eliminating the need for constrained reconstruction
    \item Co-occurrence analysis (which offering stand types appear at the same sites), yielding 
    the actual network edges needed to definitively characterize the trade network topology
    \item Correlation between guild geographic spread (number of sites) and seal frequency, 
    testing whether high frequency reflects network reach rather than local overproduction
    \item Cross-site grouping based solely on offering stand type---rather than Jamison's 
    multi-attribute criterion---to reconstruct the guild-level trade network and directly 
    test its topology for scale-free structure
\end{itemize}
